...
/// Record a failed block (or bump `retry_count` + update error info if already
/// recorded). See SKRYBITDEV-586.
        ...
        .execute(
            "INSERT INTO failed_blocks (block_height, error_kind, error_message)
             ...
             ON CONFLICT (block_height) DO UPDATE SET
                 ...
                 resolved_at = NULL",
            &[&PgNumericU64(block_height), &error_kind, &error_message],
        )
        .await
...
/// List unresolved failed blocks, ordered by block_height ascending. Used by
/// the `retry-failed` CLI. See SKRYBITDEV-586.
...
/// Count of unresolved failed blocks. See SKRYBITDEV-586.
...
/// Mark a previously-failed block as resolved. See SKRYBITDEV-586.
